...
void BanksHandler::getAllBanks(std::function<void(std::variant<BankResponse, ErrorResponse>)> callback) {
...
void BanksHandler::getBanksByCode(int code, std::function<void(std::variant<Bank, ErrorResponse>)> callback) {
  auto req = drogon::HttpRequest::newHttpRequest();
  req->setMethod(drogon::HttpMethod::Get);
