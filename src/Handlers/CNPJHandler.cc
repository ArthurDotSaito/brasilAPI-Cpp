...
void CNPJHandler::getCNPJ(std::string cnpj, std::function<void(const CNPJResponse&)> callback) {
    ...
    httpClient->sendRequest(req, [this, callback, fullUrl](drogon::ReqResult result, const drogon::HttpResponsePtr& response) {
        ...
        if (reader.parse(responseBody, jsonResponse)) {
            ...
            cnpjResponse.identificador_matriz_filial = jsonResponse["identificador_matriz_filial"].asInt64();
            ...
            cnpjResponse.situacao_cadastral = jsonResponse["situacao_cadastral"].asInt64();
            ...
            cnpjResponse.motivo_situacao_cadastral = jsonResponse["motivo_situacao_cadastral"].asInt64();
            cnpjResponse.nome_cidade_exterior = jsonResponse["nome_cidade_exterior"].asString();
            cnpjResponse.codigo_natureza_juridica = jsonResponse["codigo_natureza_juridica"].asInt64();
            cnpjResponse.data_inicio_atividade = jsonResponse["data_inicio_atividade"].asString();
            cnpjResponse.cnae_fiscal = jsonResponse["cnae_fiscal"].asInt64();
            ...
            cnpjResponse.cep = jsonResponse["cep"].asInt64();
            cnpjResponse.uf = jsonResponse["uf"].asString();
            cnpjResponse.codigo_municipio = jsonResponse["codigo_municipio"].asInt64();
            ...
            cnpjResponse.qualificacao_do_responsavel = jsonResponse["qualificacao_do_responsavel"].asInt64();
            cnpjResponse.capital_social = jsonResponse["capital_social"].asDouble();
            cnpjResponse.porte = jsonResponse["porte"].asInt64();
            ...
            if (jsonResponse.isMember("cnaes_secundarios") && jsonResponse["cnaes_secundarios"].isArray()) {
                for (const auto& cnae : jsonResponse["cnaes_secundarios"]) {
                    CNAE cnaeItem;
                    cnaeItem.codigo = cnae["codigo"].asInt();
            ...
            if (jsonResponse.isMember("qsa") && jsonResponse["qsa"].isArray()) {
                for (const auto& qsaItem : jsonResponse["qsa"]) {
                    QSA qsaMember;
                    qsaMember.identificador_de_socio = qsaItem["identificador_de_socio"].asInt();
                    qsaMember.nome_socio = qsaItem["nome_socio"].asString();
                    qsaMember.cnpj_cpf_do_socio = qsaItem["cnpj_cpf_do_socio"].asString();
                    qsaMember.codigo_qualificacao_socio = qsaItem["codigo_qualificacao_socio"].asInt();
                    qsaMember.percentual_capital_social = qsaItem["percentual_capital_social"].asInt();
                    ...
                    if (qsaItem.isMember("codigo_qualificacao_representante_legal")) qsaMember.codigo_qualificacao_representante_legal = qsaItem["codigo_qualificacao_representante_legal"].asInt();
